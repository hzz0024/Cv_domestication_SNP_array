...
####################################
setwd("~/Dropbox/Mac/Documents/HG/Domestication/17_popLDdecay/509/")
...
out = list()
chr <- seq(0,9)
pop <- c("MEW1", "MEW2", "LIW1", "LIW2", "DBW1", "DBW2", "NCW1", "NCW2", "DBX1", "DBX2", "DBX3",  "UNC1", "UNC2", "UMFS", "NEH1", "NEH2", "MEH2") # "NYH1"
for (c in chr) {
  for (p in pop){
...
for (i in 1:2)
{
  ...
  points(tmp$size.kbp, tmp$halfdecaydist, col=alpha(c("orange","skyblue")[i],0.8),pch=19,cex=1.5)
  abline(lm(halfdecaydist~size.kbp,data=tmp),col=c("orange","skyblue")[i],lwd=2)
}  

graph2ppt(file="output/halfdecay_CHRlength_500K.100bpBINS.pptx", width=18, height=6)
