...
system(paste(plink, " --vcf genetyped_data_n_539_maf05_maxmiss095_popmiss095_hwe.recode.vcf --allow-extra-chr --make-bed --out oyster1_beforeQC", sep=""))
#====================================
...
#setwd("~/Dropbox/Mac/Documents/HG/Domestication/14_ROH")
#setwd("~/Dropbox/Mac/Documents/HG/Domestication/14_ROH/DOM_n_262")

setwd("~/Dropbox/Mac/Documents/HG/Domestication/14_ROH/All_n_539_formal_run//")
...
library(plyr)
#install.packages('xlsx') # xlsx may not be loaded for macOS m1, need to install the java aarch64 https://download.oracle.com/java/17/archive/jdk-17.0.1_macos-x86_64_bin.dmg
library(xlsx)
      ...
      for (k in 1:nrow(pop)){
        ...
        #Print the current population
              ...
              for (chr in c(paste("1-",map_chr,sep=""))){
                ...
                Froh[c(1),c("Population")]<-population
                ...
                Froh[1,c("Nsnps")]<-nrow(dummy.map.basis)
                ...
                Froh[1,c("dummy_length")]<-dummy.hom_VPF$KB[1]
                
                #Write  total number of ROHs
                Froh[1,c("NROH")]<-sum(ROH.indiv$NSEG)
                
                #Write  total length of ROHs
                Froh[1,c("KBROH")]<-mean(ROH.indiv$KB)
                ...
                ROH.indiv$F_ROHall<-ROH.indiv$KB/dummy_length
                ...
                for (j in 1:nrow(fam)) {
                  ...
                  if (nrow(tmp)>0) {
                    ROH.indiv$FROH1_2[ROH.indiv$IID==iid]<-sum(tmp$KB)/dummy_length
                    ROH.indiv$NSEG1_2[ROH.indiv$IID==iid]<-nrow(tmp)
                  }
                  else {
                    ROH.indiv$FROH1_2[ROH.indiv$IID==iid]<-0
                ...
                for (j in 1:nrow(fam)) {
                  ...
                  if (nrow(tmp)>0) {
                    ROH.indiv$FROH2_4[ROH.indiv$IID==iid]<-sum(tmp$KB)/dummy_length
                    ROH.indiv$NSEG2_4[ROH.indiv$IID==iid]<-nrow(tmp)
                  }
                  else {
                    ROH.indiv$FROH2_4[ROH.indiv$IID==iid]<-0
                ...
                for (j in 1:nrow(fam)) {
                  ...
                  if (nrow(tmp)>0) {
                    ROH.indiv$FROH4_8[ROH.indiv$IID==iid]<-sum(tmp$KB)/dummy_length
                    ROH.indiv$NSEG4_8[ROH.indiv$IID==iid]<-nrow(tmp)
                  }
                  else {
                    ROH.indiv$FROH4_8[ROH.indiv$IID==iid]<-0
                ...
                for (j in 1:nrow(fam)) {
                  ...
                  if (nrow(tmp)>0) {
                    ROH.indiv$FROH8_16[ROH.indiv$IID==iid]<-sum(tmp$KB)/dummy_length
                    ROH.indiv$NSEG8_16[ROH.indiv$IID==iid]<-nrow(tmp)
                  }
                  else {
                    ROH.indiv$FROH8_16[ROH.indiv$IID==iid]<-0
                ...
                for (j in 1:nrow(fam)) {
                  ...
                  if (nrow(tmp)>0) {
                    ROH.indiv$FROH16_[ROH.indiv$IID==iid]<-sum(tmp$KB)/dummy_length
                    ROH.indiv$NSEG16_[ROH.indiv$IID==iid]<-nrow(tmp)
                  }
                  else {
                    ROH.indiv$FROH16_[ROH.indiv$IID==iid]<-0
                    ROH.indiv$NSEG16_[ROH.indiv$IID==iid]<-nrow(tmp)
                    
                  }
                }
                
                #ROH per individual when only considering runs larger than 5000kb
                ...
                #Write  FROH
                Froh[1,c("FROH")]<-mean(ROH.indiv$F_ROHall)
                
                #Write  FROH1_2
                Froh[1,c("FROH1_2")]<-mean(ROH.indiv$FROH1_2)
                
                #Write  FROH2_4
                Froh[1,c("FROH2_4")]<-mean(ROH.indiv$FROH2_4)
                
                #Write  FROH4_8
                Froh[1,c("FROH4_8")]<-mean(ROH.indiv$FROH4_8)
                
                #Write  FROH8_16
                Froh[1,c("FROH8_16")]<-mean(ROH.indiv$FROH8_16)
                ...
                ROH_animal[,c(4:10,20:22)]<-round(ROH_animal[,c(4:10,20:22)]*100,2)
                ...
                #Write  Fgrm
                Froh[1,c("Fgrm")]<-mean(grm.ibc$Fhat1)
                
                #Write  Fhom
                Froh[1,c("Fhom")]<-mean(grm.ibc$Fhat2)
                
                #Write  Funi
                Froh[1,c("Funi")]<-mean(grm.ibc$Fhat3)
                ...
                FROH_file$Mean_NROH<-FROH_file$NROH/FROH_file$Nanimals
                #calculate total kb of ROH population
                FROH_file$KBROH_total<-FROH_file$KBROH*FROH_file$Nanimals
                #calculate percentage of coverage (dummy length over genome length)
                FROH_file$perc_coverage<-FROH_file$dummy_length/genome_length
                #calculate Froh using genome length
                FROH_file$FROH_totalgenome<-FROH_file$KBROH/genome_length
                
                
                #Round results
                FROH_file[c(3,9:18,22:23)]<-round(FROH_file[c(3,9:18,22:23)]*100,2)
                
                #Write  summary file
                write.xlsx(FROH_file,file=paste(directory,"/ROH_Results_",population,".xls",sep=""),row.names = F,col.names = T,showNA = F)
                ...
                summary_ROH_breeds<-FROH_file}
                #else: rbind output from every breed
                else{summary_ROH_breeds<-rbind(summary_ROH_breeds,FROH_file)}
